  ...
  <div class="relative max-w-[32rem] w-[91.666667%] mx-auto">
    {#if onPrev}
      <button
        ...
        onclick={onPrev}
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 18l-6-6 6-6"/></svg>
      </button>
    {/if}
    {#if onNext}
      <button
			...
			<p class="text-md font-bold break-words">
				...
				{#if prop.icons}
					{#each prop.icons as icon}
						<img src={`https://cdn.opentrail.org/icons/${icon}.png`} height="25" width="25" class="inline align-middle" />
					{/each}
				{/if}
			</p>
