<script>
 ...
 convertedElevs = pd.points.map((p) => imperial ? mToFt(p.elev) : p.elev);
 }

 $: visibleMarkers = getVisibleMarkers($renderedMarkers, $data, $activeIcons);
 $: elevRange = getElevRange(convertedElevs);
 $: paths = buildLinePath(convertedElevs, elevRange, width, height);
 ...
 commentCount: f.properties.commentCount ?? (f.properties.comments || []).length
 };
 })
 ...
 const maxLabelH = h - PADDING.bottom - (PADDING.top + 2);
 const maxChars = maxLabelH > 0 ? Math.floor(maxLabelH / LABEL_CHAR_WIDTH) : 0;
 let allItems = [];
 for (const m of markers) {
 const localIdx = pd.endIdx > pd.startIdx
 ? Math.round(((m.mile * 10 - pd.startIdx) / (pd.endIdx - pd.startIdx)) * (pd.points.length - 1))
 : 0;
 if (localIdx < 0 || localIdx >= pd.points.length || m.elev == null) continue;
 const elevConverted = imp ? m.elev : ftToM(m.elev);
 ...
 const k = Math.ceil(group.length / available);
 for (let j = 0; j < group.length && kept.length < slots; j++) {
 if (j % k === 0) kept.push(group[j]);
 }
 }
 }
 ...
 let j = 0;
 for (; j <= kept.length; j++) {
 if (j === kept.length || selItem.cx < kept[j].cx) {
 kept.splice(j, 0, selItem);
 break;
 }
 }
 }
 ...
 allItems.sort((a, b) => a.cx - b.cx);
 }
 const placed = [];
 for (const item of allItems) {
 let bestX = null;
 if (item.cx - half >= plotLeft && item.cx + half <= plotRight) {
 const isFree = placed.every((p) =>
 item.cx + half <= p.labelLeft || item.cx - half >= p.labelRight
 );
 if (isFree) bestX = item.cx;
 }
 if (bestX === null) {
 let leftX = item.cx - LABEL_FONT_SIZE;
 let rightX = item.cx + LABEL_FONT_SIZE;
 while ((leftX >= plotLeft + half || rightX + half <= plotRight) && bestX === null) {
 if (leftX >= plotLeft + half && Math.abs(leftX - item.cx) <= LABEL_MAX_SHIFT) {
 const isFree = placed.every((p) =>
 leftX + half <= p.labelLeft || leftX - half >= p.labelRight
 );
 if (isFree) bestX = leftX;
 }
 if (bestX === null && rightX + half <= plotRight && Math.abs(rightX - item.cx) <= LABEL_MAX_SHIFT) {
 const isFree = placed.every((p) =>
 rightX + half <= p.labelLeft || rightX - half >= p.labelRight
 );
 if (isFree) bestX = rightX;
 }
 leftX -= LABEL_FONT_SIZE;
 rightX += LABEL_FONT_SIZE;
 }
 }
 if (bestX !== null) {
 placed.push({ id: item.id, cx: item.cx, cy: item.cy, icon: item.icon, title: item.title, labelX: bestX, labelLeft: bestX - half, labelRight: bestX + half });
 }
 }
 const result = new Map();
 for (const p of placed) {
 result.set(p.id, { cx: p.cx, cy: p.cy, selected: p.id === selId, icon: p.icon, labelX: p.labelX, title: p.title });
 }
 return result;
 }
 ...
 function getElevRange(elevs) {
 if (elevs.length === 0) return { min: 0, max: 0 };
 let min = Infinity;
 let max = -Infinity;
 ...
 const step = imperial ? 1000 : 500;
 min = Math.floor(min / step) * step;
 max = Math.ceil(max / step) * step;
 return { min, max };
 }

 function buildLinePath(elevs, er, w, h) {
 if (elevs.length === 0 || w === 0 || h === 0) return { area: '', line: '' };
 const bottom = h - PADDING.bottom;
 let line = '';
 let area = '';
 for (let i = 0; i < elevs.length; i++) {
 const x = scaleX(i, elevs.length);
 const y = scaleY(elevs[i], er.min, er.max);
 const cmd = (i === 0 ? 'M' : 'L') + x.toFixed(1) + ' ' + y.toFixed(1) + ' ';
 line += cmd;
 area += cmd;
 }
 area += 'L' + scaleX(elevs.length - 1, elevs.length).toFixed(1) + ' ' + bottom + ' ';
 area += 'L' + scaleX(0, elevs.length).toFixed(1) + ' ' + bottom + ' Z';
 return { area, line };
 }
 ...
 $: {
 const pd = $profileData;
 const idx = cursorLocalIdx;
 if (idx >= 0 && idx < pd.points.length && idx < convertedElevs.length) {
 cursorX = scaleX(idx, pd.points.length);
 cursorElev = convertedElevs[idx];
 const mile = pd.endIdx > pd.startIdx
 ? (pd.startIdx + (idx / (pd.points.length - 1)) * (pd.endIdx - pd.startIdx)) / 10
 : null;
 const dist = mile != null ? (imperial ? mile : miToKm(mile)) : null;
 cursorLabel = dist != null && cursorElev != null
 ? dist.toFixed(1) + (imperial ? 'mi' : 'km') + ' ' + Math.round(cursorElev).toLocaleString('en-US') + (imperial ? 'ft' : 'm')
 : '';
 } else {
 cursorX = -1;
 cursorElev = null;
...
<div class="elevation-profile-container" bind:this={containerEl}>
 ...
 <!-- marker dots + labels -->
 {#each [...labeledMarkers.values()] as lm}
 <circle
 cx={lm.cx}
 cy={lm.cy}
 r={lm.selected ? 4 : 3}
 fill={ICON_COLORS[lm.icon] || ICON_COLORS.o}
 stroke="white"
 stroke-width={lm.selected ? 2 : 0.5}
 ...
 transform="rotate(-90 {lm.labelX} {PADDING.top + 2})"
 style="font-size:{LABEL_FONT_SIZE}px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif"
 ...
 <!-- geo dot -->
 {#if geoDot && geoDot.localIdx < convertedElevs.length}
 <circle
 cx={scaleX(geoDot.localIdx, $profileData.points.length)}
 cy={scaleY(convertedElevs[geoDot.localIdx], elevRange.min, elevRange.max)}
 r="5"
 fill="#3b82f6"
 stroke="white"
 stroke-width="1.5"
